...
// ════════════════ REGISTRO DE PDFs GERADOS (tabela calc_comercial) ════════════════
// A coluna "calculadora" tem CHECK constraint no Supabase: só aceita os quatro
// nomes abaixo. Ao criar uma calculadora nova é preciso liberar o nome no banco
// também, senão o insert falha. Falha de registro nunca impede o PDF de sair.
const PDF_NOME = {
  contrato:  'Cálculo do contrato',
  comparar:  'Comparar planos',
  desconto:  'Simulador de desconto',
  simulador: 'Simulador de prazo personalizado'
};
function registrarPDF(tipo){
  const sb = window.giroDb || window.giroDB;
  if(!sb) return;
  const nome = PDF_NOME[tipo];
  if(!nome){ console.warn('PDF sem nome mapeado para o banco:', tipo); return; }
  const a = window.giroAuth || {};
  sb.from('calc_comercial').insert({
    calculadora:  nome,
    equipe:       S.team,
    usuario_id:   a.id   || null,
    usuario_nome: a.nome || null
  }).then(res => {
    if(res && res.error) console.warn('PDF não registrado:', res.error.message);
  }, err => {
    console.warn('PDF não registrado:', err && err.message);
  });
}
window.giroRegistrarPDF = registrarPDF;
