...
(function(){
  ...
  // ---- Supabase: lâminas cadastradas na tabela "materiais" ----
  // Nomes de coluna variam conforme a tabela; aceitamos os mais prováveis
  // e caímos no objeto MATERIAIS acima se a consulta falhar ou vier vazia.
  var DO_BANCO = null;      // { vindi:[...], bling:[...], tray:[...] }
  var buscando = null;

  function normalizarMaterial(row){
    var img = row.img || row.imagem || row.url || row.url_imagem || row.arquivo_url || row.link || '';
    var titulo = row.titulo || row.title || row.nome || 'Material';
    return {
      id: String(row.id != null ? row.id : (row.slug || titulo)),
      titulo: titulo,
      desc: row.desc || row.descricao || row.description || '',
      file: row.file || row.arquivo || row.nome_arquivo || (titulo + '.png'),
      img: img
    };
  }

  function listaDoTime(team){
    if(DO_BANCO && DO_BANCO[team] && DO_BANCO[team].length) return DO_BANCO[team];
  ...
  function buscarMateriais(){
    ...
    buscando = sb.from('materiais').select('*').then(function(res){
      if(res.error || !res.data || !res.data.length) return null;
      var por = {};
      res.data.forEach(function(row){
        var t = String(row.team || row.empresa || row.time || 'vindi').toLowerCase().trim();
        var m = normalizarMaterial(row);
        if(!m.img) return;                       // sem imagem não há o que baixar
        (por[t] = por[t] || []).push({ m:m, ordem: Number(row.ordem != null ? row.ordem : row.id) || 0 });
      });
      Object.keys(por).forEach(function(t){
        por[t] = por[t].sort(function(a,b){ return a.ordem - b.ordem; }).map(function(x){ return x.m; });
      });
      DO_BANCO = por;
      return por;
  ...
  function renderMateriais(){
    pintarGrid();
    // Se o banco responder com lâminas, redesenha por cima do conteúdo local.
    buscarMateriais().then(function(r){ if(r) pintarGrid(); });
  }
