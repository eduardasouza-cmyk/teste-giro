...
(function(){
  ...
  //
  // ATENÇÃO: a coluna "lamina" tem um CHECK constraint no Supabase que aceita
  // só os nomes curtos abaixo. Ao criar uma lâmina nova é preciso (1) incluir o
  // id aqui e (2) liberar o nome no constraint do banco — senão o insert falha.
  var LAMINA_NOME = {
    'como-funciona': 'Como funciona',
    'quem-e-quem':   'Quem é quem',
    'carencia':      'Carência',
    'pos-venda':     'Pós-venda'
  };
  ...
  function registrarDownload(team, m){
    ...
    if(!sb) return;
    var nome = LAMINA_NOME[m.id];
    if(!nome){ console.warn('Lâmina sem nome mapeado para o banco:', m.id); return; }
