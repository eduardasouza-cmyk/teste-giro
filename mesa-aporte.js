...
(function(){
  ...
  card.innerHTML =
    ...
    '<div class="card-body">'+
      ...
      '<div class="ap-filtro" id="ap-filtro">'+
        '<button type="button" class="on" data-f="hoje">Hoje</button>'+
        '<button type="button" data-f="todos">Ver todos</button>'+
        '<span class="ap-filtro-info" id="ap-filtro-info"></span>'+
      '</div>'+
  ...
  function brutoDaLinha(row){
    return row.created_at || row.criado_em || row.data || row.data_hora || null;
  }
  function dataDaLinha(row){
    const bruto = brutoDaLinha(row);
    if(!bruto) return '—';
    const d = new Date(bruto);
    return isNaN(d) ? String(bruto) : d.toLocaleString('pt-BR');
  }
  // Compara só a data (ignora a hora) no fuso do navegador.
  const diaDe = ts => { const d = new Date(ts); return isNaN(d) ? '' : d.toDateString(); };
  const hoje  = () => new Date().toDateString();
  const ehDeHoje = r => r.ts ? diaDe(r.ts) === hoje() : false;
  ...
  function calcular(){
    ...
    const reg = { cnpj, data:nowStr(), ts:new Date().toISOString(), des, fee, pix:PIX, iof, total, salvo:false };
    historico.push(reg);
    renderTabela();
  ...
  let filtro = 'hoje';   // 'hoje' | 'todos'

  function visiveis(){
    if(filtro === 'todos') return historico;
    const doDia = historico.filter(ehDeHoje);
    // Um cálculo recém-feito nesta sessão sempre aparece, mesmo sem ts do banco.
    const novos = historico.filter(r => r.salvo === false && !ehDeHoje(r));
    return doDia.concat(novos);
  }

  function renderTabela(){
    const lista = visiveis();
    const has = lista.length>0;
    $g('ap-tabela').style.display = has ? '' : 'none';
    $g('ap-empty').style.display = has ? 'none' : '';
    $g('ap-empty').textContent = historico.length
      ? 'Nenhum cálculo hoje. Use "Ver todos" para o histórico completo.'
      : 'Nenhum cálculo salvo ainda.';

    const info = $g('ap-filtro-info');
    if(info) info.textContent = (filtro === 'hoje')
      ? lista.length + ' de ' + historico.length + ' no total'
      : historico.length + ' cálculos';

    $g('ap-tbody').innerHTML = [...lista].reverse().map(r=>
  ...
  $g('ap-filtro').addEventListener('click', function(e){
    const b = e.target.closest('button[data-f]'); if(!b) return;
    filtro = b.dataset.f;
    this.querySelectorAll('button[data-f]').forEach(x=>x.classList.toggle('on', x===b));
    renderTabela();
  });
